...
import { Button, ButtonGroup, Dialog, RadioGroup, Radio, Label, FormGroup, ControlGroup, InputGroup, TextArea, Icon } from '@blueprintjs/core';
      ...
      <ControlGroup fill={true} vertical={true}>
        <div style={{marginBottom: '6px', marginTop: '6px'}}>
          {(data.regexMode === 'plain')
            ? <Tooltip2 content={'Value in textarea will be converted to pattern for RegExp constructor\nby escaping chars that would be regex control chars\n\nClick "Convert to Regex" button to edit regex pattern directly'}>PLAIN TEXT</Tooltip2>
            : <Tooltip2 content={'Value in textarea will be passed to RegExp constructor'}>REGEX CONSTRUCTOR PATTERNS</Tooltip2>
          }
        </div>
          ...
          : data.items.map(
           (d,i) => 
            <ControlGroup style={{alignItems: 'center'}} fill={true} vertical={false}>
              <Tooltip2 className='width_fixed_100px padding_left_6px' content={d.label}>{d.label}</Tooltip2>
              <InputGroup 
