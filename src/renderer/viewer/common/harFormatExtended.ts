...
/*
    THIS IS AN EXTENDED TYPE SPEC FOR THE HAR FORMAT
    INCLUDING TYPINGS FOR ITEMS ADDED BY CHROME/EDGE

    note that props starting with '_' are not part of har spec, but are added by browser

    STARTED WITH @types/har-format/index.ts
    ADDED MORE TYPES BASED ON OBSERVATION OF HAR OBJECTS EMITTED BY CHROME/EDGE
...
// type for _initiator
// added by me, based on inspection of har objects produced by chrome/edge
// notes:
//     the type specified for _initiator in the original file (@types/har-format/index.ts) is string
//     but when i inspect the actual entries that chrome provides (inspecting in mongo)
//     i find that _initiator is actually provided as an object, 
//     with a varying structure depending on the 'type' property
// THIS REPLACED THE TYPE ON _initiator IN Entry
// observed structures in captured _initiator values:
//  1)  {
//        type: 'other'
//          (nothing else)
//      }
...
//        type: 'script'
//        stack: {
//          callFrames: _initiatorCallFrame[]   (as defined below)
//          parent: {
//              description: string     (e.g., setTimeout, )
//              callFrames: _initiatorCallFrame[]
//      }
//
//  3)  {
//        type: 'parser'
...
//
//
// ALSO: ADDED OPTIONAL 'tag' PROP, FOR USE IN Pres InitView.tsx
// (for parser and callframe objects)
//
//

export type HARInitiator = _initiatorOther | _initiatorParser | _initiatorScript



export interface _initiatorOther {
    type: 'other'
}

export interface _initiatorParser {
    type: 'parser'
    url: string
    lineNumber?: number  // 0-based, not always included
    tag?: string
}

export interface _initiatorScript {
    type: 'script'
    stack?: {
        callFrames: _initiatorCallFrame[]
        parent?: _initiatorParent
    }
}

export interface _initiatorCallFrame {
    functionName: string
    scriptId: string
    url: string
...
export interface _initiatorParent {
    description: string    // description of why execution returned to queue - e.g., 'Promise.then', 'load', ... (what others do we find?)
    callFrames: _initiatorCallFrame[]
    parent?: _initiatorParent
    parentId?: {       // don't know what this is for
        id: string
        debuggerId: string
    }
}
 ...
 * entries by `startedDateTime` (starting c the oldest) is preferred way how
...
export interface Entry {
    ...
    /** _non-standard_  */
    _index?: number | null;
// replacing definition from the public spec with my own type spec based on what i see coming from chrome/edge
//    /** _non-standard_  */
...
/** added by me, based on finding this property in captures from Edge/Chrome */
...
/* added by me so that object can be indexed by array notation */
    [index: string]: any
